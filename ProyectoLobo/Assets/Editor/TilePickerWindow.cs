    ...
    void OnGUI() {
        ...
        if (selection != null) {
            ...
            if (texture2D != null) {
                ...
                scale = (Scale)EditorGUILayout.EnumPopup("Zoom", scale);//este trozo es para escalar la textura
                ...
                var viewPort = new Rect(0, 0, position.width - 5, position.height - 5); //este trozo es para que aparezcan barras de scroll a los lados si la textura es mas grande que la ventana
                ...
                var boxTex = new Texture2D(1, 1);//esto es para que la tile seleccionada se marque de azul transparente
                ...
                if (cEvent.type == EventType.mouseDown && cEvent.button == 0) {
                    ...
                    if (currentSelection.x > grid.x - 1)// esto es para que no se salga la selección de la textura
