...
public class TileMapEditor : Editor {
    ...
    {
        EditorGUILayout.BeginVertical(); //siempre que se hace un begin tiene que haber un end
        ...
        if (oldTexture != map.texture2D) { // esto soluciona un bug al crear el editor
            UpdateCalculations();
            map.tileID = 1;
            CreateBrush();
        }
        ...
        {
            EditorGUILayout.HelpBox("You have not selected a texture 2D yet.", MessageType.Warning); //Muestra un mensaje si la textura 2d no existe
        }
        else {
            ...
            if (GUILayout.Button("Clear Tiles")) {
                if (EditorUtility.DisplayDialog("Clear map's tiles?", "Are you sure madafaka?", "Yeah Baby!", "Hell no!")) {
                    ClearMap();
                }
            }
        }
    ...
    void MoveBrush() {
        ...
        if (!mouseOnMap) // revisamos el getter para ver si el mouse está dentro de la malla dibujable
            return;
